...
@torch.no_grad()
def test(hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400, use_kan=False):
    """
    Args:
    ...
    for i in range(int(np.ceil(H / chunk_size))):   # iterate over chunks
        ...
        regenerated_px_values = render_rays(model, ray_origins_, ray_directions_, hn=hn, hf=hf, nb_bins=nb_bins, use_kan=use_kan)
...
def render_rays(nerf_model, ray_origins, ray_directions, hn=0, hf=0.5, nb_bins=192, use_kan=False):
    ...
    if use_kan:
        inputs = torch.cat([x.reshape(-1, 3), ray_directions.reshape(-1, 3)], dim=-1)
...
def train(nerf_model, optimizer, scheduler, data_loader, device='cpu', hn=0, hf=1, nb_epochs=int(1e5),
    ...
    for _ in tqdm(range(nb_epochs)):
        ...
        for batch in data_loader:
            ...
            regenerated_px_values = render_rays(nerf_model, ray_origins, ray_directions, hn=hn, hf=hf, nb_bins=nb_bins, use_kan=use_kan) 
        ...
        for img_index in range(200):
            test(hn, hf, testing_dataset, img_index=img_index, nb_bins=nb_bins, H=H, W=W, use_kan=use_kan)
...
if __name__ == '__main__':
    ...
    if use_kan:
        model = KAN([6, 64, 4]).to(device) # NerfModel(hidden_dim=256).to(device)
    else:
